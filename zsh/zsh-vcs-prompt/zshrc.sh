...
if [ -z "$ZSH_VCS_PROMPT_GIT_FORMATS" ]; then
    # VCS name
    #ZSH_VCS_PROMPT_GIT_FORMATS='%{%B%F{cyan}%}(%{%B%F{white}%}#s%{%B%F{cyan}%})-%{%f%b%}'
    # Branch name
    #ZSH_VCS_PROMPT_GIT_FORMATS+='%{%B%F{cyan}%}[%{%B%F{red}%}#b%{%f%b%}'
    ZSH_VCS_PROMPT_GIT_FORMATS='%{%B%F{cyan}%}[%{%f%b%}'
    # Ahead and Behind
    ...
    # Clean
    ZSH_VCS_PROMPT_GIT_FORMATS+='%{%F{green}%}#j%{%f%b%}%{%B%F{cyan}%}]%{%f%b%}'
fi
...
#XB source $ZSH_VCS_PROMPT_DIR/lib/vcsstatus.sh
